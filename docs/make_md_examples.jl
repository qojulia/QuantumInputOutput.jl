...
# Plots/GR must stay headless in both local and CI documentation builds.
...
# Each task owns a separate Julia process. `asyncmap` therefore provides
# bounded concurrency without sharing modules, solver state, plotting state,
# or generated files between examples.
